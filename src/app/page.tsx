...
const InvoiceData = ({invoiceID}: {invoiceID: string | null}) => {
  return(
    <div className="flex flex-col items-start rounded-xl h-full w-full shadow-md">
      <div className="flex flex-row justify-between w-full bg-gray-100">
        {invoiceID && <img src={`/invoice${invoiceID}.jpg`} alt={`invoice ${invoiceID}`} className="w-full h-full" />}
      </div>
    </div>
  )
}
...
const DemoComponent = () => {
  ...
  const InvoiceShowcase = ({ invoiceID }: { invoiceID: string | null }) => {
    return(
      <div className="flex flex-col items-start gap-0 border-2 border-gray-200 rounded-xl h-full w-full shadow-md">
        <div className="flex flex-row justify-between w-full bg-gray-100">
  ...
  return (
    <div className="absolute top-10 right-20 flex flex-col items-start gap-10 border-2 border-gray-200 rounded-2xl h-[700px] w-[1000px] shadow-xl">
      <div className="flex flex-col w-full">
        <div className="bg-white p-6">
          <div className="flex flex-row gap-6">
            <div onDrop={handleDrop} onDragOver={handleDragOver} className={`relative w-1/2 p-4 h-full border border-dashed border-gray-300 rounded-lg bg-gray-50 flex flex-col gap-4 justify-center items-center ${isDragging ? "opacity-50" : "opacity-100"}`}>
              {isDragging && isDropped && (
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col justify-center items-center text-gray-900 text-xl text-md">Drop invoice here</div>
              )}
              
              {isDropped ? (
                <InvoiceShowcase invoiceID={selectedInvoice}/>
              ) : (
                <p className="text-gray-900 text-xl">Drop invoice here</p>
              )}
            </div>
            <div className="flex flex-col w-1/2  p-4 rounded-lg">
              <div className="flex flex-col gap-4 bg-gray-50 p-4 mb-5">
                <div className="flex justify-between items-center">
                  <h3 className="font-semibold text-gray-900">Extracted Details</h3>
                  <span className="text-sm text-emerald-600">{loadingProgress}% Complete</span>
                </div>
                    ...
                    {loadingProgress === 100 ? (
                      <>
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{selectedInvoiceData?.InvoiceNumber}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-600">PO Number</span>
                          <span className="font-medium">{selectedInvoiceData?.PO}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-600">Invoice Date</span>
                          <span className="font-medium">{selectedInvoiceData?.issued_date}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-600">Due Date</span>
                          <span className="font-medium">{selectedInvoiceData?.due_date}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-600">Supplier</span>
                          <span className="font-medium">{selectedInvoiceData?.Supplier}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{selectedInvoiceData?.tax}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-600">Total Amount</span>
                          <span className="font-medium font-semibold">{selectedInvoiceData?.total}</span>
                        </div>
                      </>
                    ) : (
                ...
                <div className="h-1 bg-emerald-100 rounded-full">
                  <div 
                    className={`h-1 bg-emerald-500 rounded-full ${loadingProgress === 0 ? '' : 'transition-all duration-200'}`}
                ...
                <div className="flex flex-row gap-3 p-5">
                  {sampleInvoices.map((invoice) => (
                    ...
                    >
                      ...
                      <span className="text-sm text-gray-500">{invoice.issued_date}</span>
                    </div>
                  ))}
                </div>
                <div className="flex flex-row justify-center text-gray-500 text-sm"> Drag these</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div>
      ...
      <main>
        <div className="flex flex-col">
          <section className="relative flex flex-row gap-20 items-start py-40 px-5 ms-[500px]">
